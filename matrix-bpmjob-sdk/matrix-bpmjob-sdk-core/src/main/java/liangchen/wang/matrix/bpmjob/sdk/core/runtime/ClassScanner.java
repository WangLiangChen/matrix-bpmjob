package liangchen.wang.matrix.bpmjob.sdk.core.runtime;
...
public enum ClassScanner {
    ...
    private static void scanAllClassPath() throws Exception {
        String[] javaClassPaths = System.getProperty(JAVA_CLASS_PATH).split(";");
    ...
    private static String classPath2ClassName(String classPath) {
        String className = classPath.replaceAll(Matcher.quoteReplacement(System.getProperty("file.separator")), Matcher.quoteReplacement("."));
    ...
    public <A extends Annotation> Set<Method> getAnnotatedMethods(Class<A> annotationClass) {
        return classes.stream().flatMap(clazz -> Arrays.stream(clazz.getDeclaredMethods()))
                .filter(method -> null != method.getAnnotation(annotationClass) && Modifier.isPublic(method.getModifiers())).collect(Collectors.toSet());
    }
